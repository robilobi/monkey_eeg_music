...
preproc_path = ([U 'eeg\TRF_PITCHONSET\']); mkdir(preproc_path); % output folder
% To get stim info
...
% (20-4-2022) The stimulus onset always is 5 s relative to the
    ...
    for sbj = 1:2
        ...
        for s = 1:length(stim_names)
           ...
           idyom_v = idyom_v(1:length(spflux),:);
            ...
            stim_AM{s} = [spflux idyom_v(:,1) idyom_v(:,4:7)]; % all musical features
            stim_AMp{s} = [spflux idyom_v(:,1) idyom_v(:,[4 6])]; % Sp Ep
            stim_AMo{s} = [spflux idyom_v(:,1) idyom_v(:,[5 7])]; % So Eo
        end
        ...
        figure
        hold on
        subplot(4,1,1);plot(t,mel(:,2),'k','LineWidth',1.5);   set(gca,'FontSize',10,'XLim',[0 10]);
        subplot(4,1,2);plot(t,mel(:,4),'k','LineWidth',1.5);   set(gca,'FontSize',10,'XLim',[0 10]);
        xlabel('Time (s)');
        ...
        % AMl model
        [statsall_AMp,mdl_AMp] = itercvtest(stim_AMp,eeg,Fs,1,minlag,maxlag,lambdas);
        % AMshu model
        [statsall_AMo,mdl_AMo] = itercvtest(stim_AMo,eeg,Fs,1,minlag,maxlag,lambdas);
